                ...
                <div id="mundo_aventura">
                    <div class="container">
                        <div class="card-deck">
                            ...
                            // Construye la consulta SQL para obtener establecimientos aprobados por la localidad
                            $consultaEstablecimientos = "SELECT * FROM registro_de_establecimiento WHERE Aprobado = 1 AND localidad = '$localidad'";

                            // Ejecuta la consulta de establecimientos
                            $resultadoEstablecimientos = $conexion->query($consultaEstablecimientos);

                            // Verifica si hay resultados
                            if ($resultadoEstablecimientos->num_rows > 0) {
                                // Almacena los resultados en un array
                                $establecimientos = $resultadoEstablecimientos->fetch_all(MYSQLI_ASSOC);

                                // Itera sobre los establecimientos
                                foreach ($establecimientos as $establecimiento):
                                    // Obtiene el ID del establecimiento
                                    $idEstablecimiento = $establecimiento['Id_registro'];

                                    // Construye la consulta SQL para obtener hasta 10 imágenes asociadas a cada establecimiento
                                    $consultaImagenes = "SELECT * FROM imagenes_establecimiento WHERE id_establecimiento = $idEstablecimiento LIMIT 10";

                                    // Ejecuta la consulta de imágenes
                                    $resultadoImagenes = $conexion->query($consultaImagenes);
                                    ...
                                    <div class="col-md-4">
                                        <div class="card">
                                            ...
                                            <!-- Muestra hasta la primera imagen asociada al establecimiento -->
                                            <?php
                                            if (!empty($imagenes)) {
                                                $imagen = $imagenes[0];
                                                $imagenUrl = $imagen['ruta_destino'];
                                            } else {
                                                // Puedes establecer una imagen predeterminada si no hay imágenes
                                                $imagenUrl = 'ruta de la imagen predeterminada';
                                            }
                                            ?>
                                            <img class="card-img-top" src="<?php echo $imagenUrl; ?>"
                                                alt="<?php echo htmlspecialchars($establecimiento['Nombre_del_establecimiento']); ?>">

                                            <!-- Cuerpo de la tarjeta -->
                                            <div class="card-body">
                                                <h5 class="card-title">
                                                    <?php echo htmlspecialchars($establecimiento['Nombre_del_establecimiento']); ?>
                                                </h5>
                                                <h6 class="card-subtitle mb-2 text-muted">
                                                    <?php echo str_replace(['~', '¬'], ' ', htmlspecialchars($establecimiento['Direccion_de_establecimiento'])); ?>
                                                </h6>
                                                <!-- Enlace para abrir informacion.php con el ID del establecimiento en la URL -->
                                                <a href="resultado2.php?tabla=registro_de_establecimiento&id=<?php echo $idEstablecimiento; ?>"
                                ...
                                echo '<p>No hay establecimientos aprobados para la localidad seleccionada.</p>';
                            }
                            ?>
                        </div>
                    </div>
